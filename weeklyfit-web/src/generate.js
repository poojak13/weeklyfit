...
function buildPrompt(profile, weights, lastPlan, feedback, pantry, eatOut, eatOutCuisine) {
  ...
  return [
    ...
    '',
    'For guided workouts (Yoga, Weight lifting, HIIT, Pilates): give a specific ytSuggestion (video title or series from their preferred channel). If no channel specified, suggest a well-known free resource.',
    'For Weight lifting sessions: always specify (a) which split — Push / Pull / Legs, (b) duration in minutes e.g. "45 min Push day — chest, shoulders, triceps", (c) ytSuggestion e.g. "Jeff Nippard – Push Day (PPL Series)"',
    'Every workout entry MUST include: modality, detail (with duration e.g. "30 min"), focus (e.g. "Push", "Pull", "Legs", "upper body", "cardio endurance"), ytSuggestion (for guided types), steps (estimated).',
    ...
    `Example day object with ONLY the user's slots (${slots.join(', ')}):`,
    JSON.stringify({
      day: 'Mon',
...
export async function generatePlan(profile, weights, lastPlan, feedback, pantry, eatOut, eatOutCuisine, apiProvider, apiKey) {
  ...
  const strengthMods = ['Weight lifting','HIIT','Strength','Weights','Weight Training','Resistance','Strength Training','Weightlifting']
  const cardioMods = ['Running','Swimming','Treadmill','Cycling','Rowing','Elliptical','Jump rope','Cardio','Jogging','Biking','Walking briskly']
  const flexMods = ['Yoga','Stretching','Pilates','Dance','Flexibility','Mobility','Martial arts']
  ...
  // ENFORCE slots in code — strip any meal keys the user didn't ask for
  // This is the only reliable way — AI instructions alone aren't enough
  plan.days.forEach(d => {
    if (!slots.includes('breakfast')) delete d.breakfast
    if (!slots.includes('lunch')) delete d.lunch
    if (!slots.includes('dinner')) delete d.dinner
  })
